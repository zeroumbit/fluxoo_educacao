...
async function seed() {
  console.log('Iniciando restauração de modelos de autorização...');
  
  // Como o usuário pediu que NENHUM apareça como ativo por padrão:
  const modelosParaInserir = modelos.map(m => ({
    ...m,
    tenant_id: null, // Global
    ativa: false     // Inativo por padrão conforme pedido
  }));

  const { data, error } = await supabase
    .from('autorizacoes_modelos')
    .insert(modelosParaInserir);

  if (error) {
    console.error('Erro ao inserir modelos:', error);
  } else {
    console.log('Modelos globais restaurados com sucesso (todos inativos).');
  }
}
