...
function FieldInfo({ text }: { text: string }) {
  return (
    <span className="group relative cursor-help">
      <Info size={13} className="text-slate-400 hover:text-teal-500 transition-colors" />
    ...
    <div className="space-y-1.5">
      ...
      <div className="relative">
        <select
          ...
          className="appearance-none h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm shadow-sm focus:outline-none focus:border-teal-400 focus:ring-2 focus:ring-teal-50 transition-all"
        >
          {options.map((o) => (
    ...
    <div className="space-y-1.5">
      ...
      <Input
        ...
        className={cn(
          ...
          error ? 'border-red-400 focus-visible:ring-red-100' : 'focus-visible:ring-teal-100 focus-visible:border-teal-400'
        )}
      />
    </div>
  )
...
function ToggleRow({ label, description, checked, onCheckedChange, risk }: {
  ...
  return (
    <div className="flex items-center justify-between p-4 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-white transition-colors">
      ...
      <Switch checked={checked} onCheckedChange={onCheckedChange} className="data-[state=checked]:bg-teal-600 shrink-0" />
    </div>
  )
}
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <Button
          ...
          className="bg-teal-600 hover:bg-teal-700 text-white gap-2 h-10 px-6 rounded-xl font-bold shadow-sm shadow-teal-200 shrink-0"
        >
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className={cn('text-[10px] font-bold uppercase tracking-wider', isActive ? 'text-teal-200' : 'text-slate-400')}>
                    {tab.sublabel}
                  </div>
                </div>
                {isActive && <ChevronRight size={14} className="text-teal-300 shrink-0" />}
              </button>
            )
          })}
        </nav>
              ...
              <SectionCard icon={Banknote} title="Métodos de Recebimento">
                <div className="space-y-3">
                  ...
                  {financeira.pix_manual && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-4 border-l-2 border-teal-200 py-2 animate-in slide-in-from-top-2 duration-200">
                      <div className="space-y-1.5">
                  ...
                  <div className="flex items-center justify-between p-4 rounded-xl border border-teal-100 bg-teal-50/40">
                    <div>
                      <p className="text-sm font-bold text-slate-800 flex items-center gap-2">
                        PIX Automático (API)
                        <Badge className="bg-teal-600 text-white border-0 text-[9px] h-4">Recomendado</Badge>
                      </p>
                      <p className="text-xs text-slate-500">QR Code dinâmico com baixa automática de faturas.</p>
                    </div>
                    <Switch checked={financeira.pix_auto} onCheckedChange={(v) => setFinanceira({ ...financeira, pix_auto: v })} className="data-[state=checked]:bg-teal-600" />
                  </div>
                    ...
                    <div key={h.id} className="flex items-start gap-3 p-3 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-white transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          <span className="text-[10px] font-black uppercase text-teal-600 tracking-wider">{h.categoria}</span>
